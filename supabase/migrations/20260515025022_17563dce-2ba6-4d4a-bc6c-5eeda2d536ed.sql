...
-- Rate limiting para cidadao_consultar (consulta pública)
CREATE TABLE IF NOT EXISTS public.cidadao_consulta_tentativas (
...
CREATE INDEX IF NOT EXISTS idx_cct_cpf_time ON public.cidadao_consulta_tentativas (cpf, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_cct_ip_time ON public.cidadao_consulta_tentativas (ip, created_at DESC);

ALTER TABLE public.cidadao_consulta_tentativas ENABLE ROW LEVEL SECURITY;

-- Apenas admin pode visualizar
CREATE POLICY cct_admin_select ON public.cidadao_consulta_tentativas
  FOR SELECT TO authenticated
  USING (private.has_role(auth.uid(), 'admin'::app_role));

-- Recriar cidadao_consultar com rate limiting embutido
...
BEGIN
  ...
  -- Rate limit: 10 falhas/hora por CPF
  ...
  -- Rate limit: 30 falhas/hora por IP
...
-- Garante permissão (a função continua acessível para anon)
...
-- Limpa tentativas antigas (>30 dias) automaticamente via política simples:
-- (sem pg_cron; opcional para o futuro)
